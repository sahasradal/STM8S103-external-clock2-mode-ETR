stm8/
	; not working on TSSOP20 ver chip as TIM1_ETR is on PB3, only available on 32 pin version
	#include "mapping.inc"
...
timer4_ISR
	...
	ldw x,counter1		; load counter value to x
	subw x,#1			; decrease 1 for each interrupt = 2ms
	tnzw X				; test if X = 0
	jreq reload_counter	; if x = 0(counter =0) jump to reload_counter label
	ldw counter1,x		; else store decreased value back to counter1
	iret				; return from interrupt
reload_counter
	ld a,TIM1_CNTRH		; reach here if counter reached 0, load A timer1 counter MSB
	ld captureH,a		; store in captureH
	ld a,TIM1_CNTRL		; load A timer1 counter LSB
	ld captureL,a		; store in captureL
	mov buffers,capture_state	; capture_state variable has number of timer1 counter overflows
	bres TIM1_CR1,#0	; disable timer1
	ld a,#0				; load 0 in A
	ld TIM1_CNTRH,a		; reset TIM1counter msb
	ld TIM1_CNTRL,a		; reset TIM1counter lsb	
	ld capture_state,a  ; tim1 counter overflow cont register
	ldw x,#500			; load x 500, as each interrupt is 2ms , 500 interrupts willcount 1000ms
	ldw counter1,x		; load 500 counts in counter1
	mov state,#1		; load 1 in state, indicates 1 second has elapsed to the main program loop
	iret				; return from interrupt
...
TIM1_ISR
	...
	inc capture_state	; each count of this register indicates the number of timer1 overflow in 1 second
	iret				; return from interrupt
